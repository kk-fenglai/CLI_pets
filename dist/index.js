...
function profileText(bones, soul) {
    ...
    const stats = STAT_NAMES.map(s => `  ${s.padEnd(10)} ${statBar(bones.stats[s])} ${String(bones.stats[s]).padStart(3)}`).join('\n');
    return [
        sprite,
        '',
...
server.registerTool('companion_get', {
    title: 'Get companion',
    description: 'Show your terminal companion: its ASCII sprite, species, rarity, hat and stats. ' +
...
server.registerTool('companion_stats', {
    ...
    const lines = STAT_NAMES.map(s => `${s.padEnd(10)} ${statBar(bones.stats[s])} ${String(bones.stats[s]).padStart(3)}/100`);
    return text([
