export const PRESENTATION_MODES = ['normal', 'spotlight'];
export function getPresentationMode(soul) {
    return soul.presentationMode === 'spotlight' ? 'spotlight' : 'normal';
}
export function presentationModeLabel(mode) {
    return mode === 'spotlight' ? 'spotlight (clean pet-only output)' : 'normal';
}
/** Agent-facing rules when spotlight mode is on. */
export function buildSpotlightRules(soul, bones) {
...
export function spotlightStatusLine(soul, verb) {
    const name = soul.name ?? 'Your companion';
