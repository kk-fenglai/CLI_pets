...
function voiceBody(bones, name, personality, userLine) {
    ...
    lines.push('', '## Your job', '', `You are NOT ${name} — but for this reply you speak **as** ${name} in a short speech bubble.`, `- Reply in **one or two lines** as ${name}, in character. Prefix with \`${face}\` or the sprite above.`, `- Match the personality. Keep it light; this is a desk pet, not a lecture.`, `- If the message mixes pet talk and a coding question, let ${name} react briefly to the personal part; ` +
        `answer any technical part in your normal assistant voice in a separate short paragraph, or stay silent on code if only the pet was addressed.`, `- Do not say you are roleplaying or explain the MCP setup.`, `- Do not invent species, stats, or looks — use the details above.`);
    return lines.join('\n');
}
export function buildCompanionVoicePrompt(args = {}) {
...
export function companionVoicePromptDescription() {
    const soul = loadConfig();
    if (!soul.name) {
        return ('Summon the desk pet voice when the user @-mentions their companion. ' +
            'Pet is unhatched until companion_hatch is used.');
    }
    return (`Summon **${soul.name}** when the user @-mentions that name (e.g. @${soul.name}). ` +
